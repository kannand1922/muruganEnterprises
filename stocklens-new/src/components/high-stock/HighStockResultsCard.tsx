...
import type { HighStockProductRow, ShopLocation } from "../../api/metaApi";
import { formatPackLabel, normalizeSearchValue, toPackSortValue } from "../low-stock/lowStockUtils";

type HighStockResultsCardProps = {
  title?: string;
...
export function HighStockResultsCard({
  title,
  rows,
  loading = false,
  locations,
  ...
  onSelectLocation,
  onRefresh,
  emptyMessage = "No high stock products for current filters.",
}: HighStockResultsCardProps) {
  ...
  return (
    <IonCard className="settings-config-card">
      ...
      <IonCardContent>
        ...
        {loading ? <IonText color="medium">Updating high stock list...</IonText> : null}
            ...
            {filteredRows.map((row) => (
              <div key={`${row.itemCode}_${row.packValue}`} className="low-stock-result-row">
                <div className="low-stock-result-head">
                  ...
                  <IonBadge color="warning">{row.ruleType === "product" ? "Product Rule" : "Pack Rule"}</IonBadge>
                </div>
                <div className="low-stock-result-meta">
                  Code: {row.itemCode} | Pack: {formatPackLabel(row.packValue)}
                </div>
                <div className="low-stock-result-meta">
                  Current: {row.currentBottles} | Threshold: {row.thresholdBottles} | Excess: {row.excessBottles}
                </div>
              </div>
            ))}
          </div>
