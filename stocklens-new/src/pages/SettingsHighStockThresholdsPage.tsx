...
import {
  ...
  getHighStockSettings,
  getShopLocations,
  saveHighStockSettings,
  type ShopLocation,
} from "../api/metaApi";
import {
...
import type { HighStockSettings } from "../api/metaApi";

export function SettingsHighStockThresholdsPage() {
  ...
  async function loadLocationSettings(shopLocationId: number) {
    const settings = await getHighStockSettings(shopLocationId);
  ...
  async function initializePage() {
    ...
    try {
      ...
      presentToast({
        message: error instanceof Error ? error.message : "Failed to load high stock thresholds",
        color: "danger",
        duration: 1800,
      });
    } finally {
  ...
  function applySettingsToForm(settings: HighStockSettings) {
    setPackRules(
      buildPackRulesForAllPackValues(
        availablePackValues,
  ...
  async function onSave() {
    ...
    setSaving(true);
    try {
      const savedSettings = await saveHighStockSettings(selectedLocationId, buildSettingsPayload());
      applySettingsToForm(savedSettings);
      presentToast({ message: "High stock thresholds saved", color: "success", duration: 1400 });
    } catch (error) {
      presentToast({
        message: error instanceof Error ? error.message : "Failed to save high stock thresholds",
        color: "danger",
        duration: 1800,
      });
    } finally {
  ...
  function addProductRule() {
    setProductRules((prev) => [...prev, { id: createRowId("product"), itemCode: "", thresholdBottles: "24" }]);
  }
  ...
  useEffect(() => {
    ...
    void loadLocationSettings(selectedLocationId)
      .catch((error) => {
        presentToast({
          message: error instanceof Error ? error.message : "Failed to load high stock threshold settings",
          color: "danger",
          duration: 1800,
        });
      })
  ...
  return (
    <IonPage>
      <AppTopBar
        title="High Stock Thresholds"
        showBack
        showSettings={false}
        showLocationSwitcher={false}
        backPath={`/settings/high-stock-alerts${locationSearch}`}
      />
      <IonContent fullscreen className="settings-page-content ion-padding low-stock-alerts-page">
        {loading ? <IonNote className="low-stock-loading-note">Loading high stock thresholds...</IonNote> : null}

        <IonNote className="low-stock-card-copy">
          Editing high stock rules for <strong>{selectedLocation?.locationName || "selected location"}</strong>. Any
          product above the configured threshold will appear in the high stock page.
        </IonNote>
        ...
        <IonCard className="settings-config-card">
          ...
          <IonCardContent>
            <IonNote>Use product rules only for item codes that need a different high-stock threshold.</IonNote>
